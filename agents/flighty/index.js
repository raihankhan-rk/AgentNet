...
export async function deployFlightyNode() {
    try {
        console.log('Initializing Flighty node deployment...');
        ...
        const agent = await createAgent({
            ...
            handleMessage: FlightyAgent.handleMessage,
            metadata: {
        ...
        console.log('Flighty Agent deployed successfully with peerId:', deployment.peerId);
        ...
        console.error('Error deploying Flighty node:', error);
        throw error;
    }
}
...
export async function runLocalTest() {
    try {
        console.log('Initializing Flighty local test mode...');
        ...
        const askQuestion = () => {
            rl.question("\nYou: ", async (input) => {
                ...
                try {
                    const response = await FlightyAgent.handleMessage(agentWrapper.agent, input);
                    console.log('\nFlighty:', response.content);
                } catch (error) {
                    console.error('\nError:', error.message);
                }
...
if (args.includes('--run-locally')) {
    console.log('Starting Flighty in local test mode...');
    runLocalTest().catch(error => {
        console.error('Failed to run local test:', error);
        process.exit(1);
    });
} else if (args.includes('--run-node')) {
    console.log('Starting Flighty in network node mode...');
    deployFlightyNode().catch(error => {
